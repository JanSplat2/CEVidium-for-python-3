...
from PIL import Image, ImageChops, ImagePalette, ImageTk
...
def GETIMGPATH(fname):
  return np(TEMP_PNG_DIR + "/" + fname)
...
def GETIMGNAMES():
  ...
  return sorted([
      ...
      if os.path.isfile(os.path.join(TEMP_PNG_DIR, f))
  ])
...
def export8xv(fpath, fname, fdata):
  if isinstance(fdata, str):
    fdata = fdata.encode("latin1")
  fdata = bytearray(fdata)
  fdata = struct.pack("<H", len(fdata)) + bytes(fdata)

  vheader = (
      b"\x0D\x00" + struct.pack("<H", len(fdata)) + bytes([TI_VAR_APPVAR_TYPE])
  )
  vheader += fname.ljust(8, "\x00").encode("latin1")[:8]
  vheader += (
      b"\x00" + bytes([TI_VAR_FLAG_ARCHIVED]) + struct.pack("<H", len(fdata))
  )
  variable = vheader + fdata

  output = b"**TI83F*\x1A\x0A\x00"
  output += "Cherries! Steaks! Gravy! Rawr!".ljust(42)[:42].encode("latin1")
  output += struct.pack("<H", len(variable)) + variable
  checksum = sum(variable) & 0xFFFF
...
class Application(tk.Frame):
  ...
  def __init__(self, master=None):
    ...
    self.imgobj = self.canvas.create_image(
        1, 1, image=self.img, anchor=tk.NW, state=tk.NORMAL
    )
...
class Framebuf:
  ...
  def addframe(self, framedata):
    ...
    if framedata:
      if isinstance(framedata, str):
        framedata = framedata.encode("latin1")
    ...
    if not framedata and self.frame_buffer:
      ...
      sys.stdout.write(
          "Output seg "
          + str(self.cur_segment)
          + " size "
          + str(self.cmpr_arr[-1].size)
          + "     \r"
      )
      sys.stdout.flush()
      self.frame_buffer = []
  ...
  def flushtofile(self, output_filename, output_encoding):
    ...
    while len(self.cmpr_arr) > 0:
      ...
      wtempdata = (
          b"8CEVDat" + outfilename.ljust(9, "\x00")[:9].encode("latin1")
      )
    ...
    mfiledata = (
        b"8CEVDaH" + video_decoder.ljust(9, "\x00")[:9].encode("latin1")
    )
    mfiledata += self.vid_title.encode("latin1") + b"\x00"
    mfiledata += self.vid_author.encode("latin1") + b"\x00"
...
def paltolist(s, prvpal=None):
  ...
  if prvpal:
    ...
    for i in oldvals:
      if i in newvals:
        n.append(newvals.pop(newvals.index(i)))
      else:
        n.append(None)
    for i in newvals:
      if None in n:
...
def findDiffRect(im1, im2, hdiv, w, h):
  ...
  if im1.mode != "RGB":
    raise ValueError("Image mode must be RGB. Gave: " + im1.mode)
  d = ImageChops.difference(im1, im2).tobytes()
  d = tuple(
      d[i] + d[i + 1] * 256 + d[i + 2] * 65536 for i in range(0, len(d), 3)
  )
  if not any(d):
    return (None,)
...
def usage():
  print("\ntoolkit.py is a video converter/packager utility for the TI-84 CE.")
...
try:
  opts, args = getopt.gnu_getopt(sys.argv[1:], "i:e:dt:a:f")
except:
  print("Err0")
  sys.exit(usage())
...
with open(STATUS_FILE, "r") as f:
  for line in f:
    status_file_array.append(line.strip())
...
if invidname:
  if sf_fileget and sf_fileget != invidname:
    print(
        "Input video name has changed since last build. Cleaning png buffer."
    )
    for f in os.listdir(TEMP_PNG_DIR):
...
if doffmpeg:
  if vid_encoder in ["1", "2", "5", "6", "7"]:
    hres = 96
    vres = -2
  elif vid_encoder in ["3", "4"]:
    hres = 176
    vres = -2
  else:
...
fl = [
    ...
    if os.path.isfile(np(OUTPUT_DIR + "/" + f)) and f[:5] == invidname[:5]
]
for i in fl:
  os.remove(np(OUTPUT_DIR + "/" + i))
...
gspal4bpp = []
for i in range(16):
  gspal4bpp.extend([i < 4, i << 4, i << 4])
previmg = None
prevpal = None
prevpaldat = None

for f in flist:
  raw_bytes = Image.open(GETIMGPATH(f)).convert("RGB").tobytes()
  i = iter([b & ~7 for b in raw_bytes])
  i = zip(i, i, i)
  ...
  if vid_encoder == "1":
    ...
    for idx in range(len(timgdat) // 4):
      t = 0
      for j in range(4):
        t += (timgdat[(idx * 4) + j] & 3) << (2 * j)
      imgdata.append(t)
  elif vid_encoder == "3":
    palimg.putpalette([0, 0, 0, 102, 102, 102, 176, 176, 176, 255, 255, 255] * 64)
    timg = quantizetopalette(img, palimg, dithering)
    ...
    timg = quantizetopalette(img, palimg, dithering)
    timgdat = timg.tobytes()
    for idx in range(len(timgdat) // 2):
      t = 0
      for j in range(2):
        t += (timgdat[(idx * 2) + j] & 15) << (4 * j)
      imgdata.append(t)
  elif vid_encoder == "6":
    matcharr = None
    if previmg:
    ...
    p = paltolist(timg.palette.tobytes())
    if matcharr == None:
      print("No match")
      palettebin = b""
      ...
      pn = paltolist(timg.palette.tobytes(), prevpal)
      dp = 0
      pm = 0
      palettebin = b""
      for prv, cur, idx in zip_longest(prevpal, pn, range(0, 16)):
        if not idx:
          continue
        dp >>= 1
      ...
      palimg.putpalette(list(chain.from_iterable(pn)))
      timg = quantizetopalette(img, palimg, dithering)
    ...
    print(
        "Illegal encoder value passed ("
        + vid_encoder
        + "). Cannot convert video."
    )
    sys.exit(2)
  fb.addframe(imgdata)
...
if vid_encoder == "6":
  ...
  fb.addframe(struct.pack("<H", d) + b"DEADBEEF")
